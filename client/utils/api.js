import axios from "axios";
import { parseCookies } from "nookies";

export const url = `http://localhost:1337`;

const token = ``;

// let auth = parseCookies();
let auth;

const API = axios.create({
  baseURL: url,
  headers: {
    Authorization: auth ? `Bearer ${auth}` : `Bearer ${token}`
  },
});

export const fetchCategories = async () =>
  API.get(`/api/categories?populate=*`);

export const postImage = async (data) => API.post(`/api/upload`, data);

export const register = async (data) => {
  try {
    const res = await API.post(`/api/auth/local/register`, data);
    return res.data;
  } catch (err) {
    return err.response.data.error.message;
  }
};
export const loginUser = async (data) => {
  try {
    const res = await API.post(`/api/auth/local`, data);
    return res.data;
  } catch (err) {
    return err.response.data.error.message;
  }
};

export const fetchArticles = async () => API.get(`/api/articles?populate=*`);

export const fetchUser = async (value) =>
  API.get(`/api/users?filters=[email][$eq]=${value}&populate=*`);

export const fetchAllUsers = async () => API.get(`/api/users?populate=*`);

export const fetchSingleArticle = async (value) =>
  API.get(`api/articles?filters[Slug][$eq]=${value}&populate=*`);
